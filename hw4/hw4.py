...
def maximization(points_list, ranks):
    ...
    w_new = ( 1 / N ) * ranks.sum(axis=0)
    mu_new = (ranks * points_np[:, None]).sum(axis=0)* (1/w_new)* (1/N)
    sigma_new =np.sqrt((( np.power((points_np[:, None] - mu_new),2))* ranks).sum(axis=0) * (1/w_new)* (1/N))  
